...
(defn init-map! [owner props]
  (let [node (om/get-node owner "map")
        ...
        map (ol.Map. #js {:layers #js [raster vectorLayer]
                          :target node
                          :view   view})]
    (when-let [c (:center props)]
      (.setCenter view (-> c clj->js
    ...
    (doto owner
      ...
      (om/set-state! :interactions [dragBox select scale translate hover])
...
(defn BoxMap [{:keys [on-mark-change mark-change-debounce disabled] :as props} owner]
  (reify
    ...
    om/IDidUpdate
    (did-update [_ {prev-disabled :disabled} _]
      (when (not= disabled prev-disabled)
        (let [map (om/get-state owner :map)
              f (if disabled
                  #(.removeInteraction map %)
                  #(.addInteraction map %))]
          (doseq [i (om/get-state owner :interactions)]
            (f i)))))
